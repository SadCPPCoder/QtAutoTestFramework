...
### Linux
- Ubuntu 20.04+ / Fedora 36+ / 其他主流发行版
- Qt 6.5+
- CMake 3.16+
- GCC 11+ 或 Clang 14+
- libmicrohttpd 开发库
...
无需额外依赖，使用系统自带的 HTTP Server API。
...
1. 打开 Qt Creator
2. 文件 → 打开文件或项目 → 选择 `cpp-server-lib/CMakeLists.txt`
3. 选择 Qt 6.5.3 套件
4. 点击构建按钮
...
| 组件 | Windows | Linux |
|------|---------|-------|
| HTTP 服务器 | Windows HTTP Server API | libmicrohttpd |
| 线程模型 | QThread 阻塞等待 | MHD 内部线程 + Qt 信号 |
| 编译依赖 | Httpapi.lib | libmicrohttpd |
...
# 指定端口
./YourApp --test-server --test-port=9090

# 完整配置
./YourApp --test-server --test-port=8080 --test-log-level=DEBUG --test-log-file=debug.log
```
...
[2026-06-19 15:03:09.844] [INFO] HTTP Server started on port 8080 (Windows)
...
**Linux:**
端口 1024 以下需要 root 权限，建议使用 8080 或更高端口。
