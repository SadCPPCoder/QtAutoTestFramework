...
bool HttpServer::startServer(quint16 port)
{
    ...
    result = HttpCreateHttpHandle(&m_requestQueue, 0);
    if (result != NO_ERROR) {
    ...
    result = HttpAddUrl(m_requestQueue, (PCWSTR)url.utf16(), nullptr);
    if (result != NO_ERROR) {
        LOG_ERROR(QString("HttpAddUrl failed: %1").arg(result));
        CloseHandle(m_requestQueue);
        HttpTerminate(HTTP_INITIALIZE_SERVER, nullptr);
        return false;
    }
...
void HttpServer::stop()
{
    ...
    if (m_requestQueue != INVALID_HANDLE_VALUE) {
        CloseHandle(m_requestQueue);
        m_requestQueue = INVALID_HANDLE_VALUE;
    }
...
void HttpServer::run()
{
    ...
    while (m_running) {
        ...
        if (result == NO_ERROR) {
            QString method = parseMethod(request->Verb);
            QString path = parseUrl(request->CookedUrl);
            quint64 requestId = request->RequestId;

            QByteArray body;
            if (method == "POST" || method == "PUT") {
                body = receiveBody(m_requestQueue, requestId);
            }
...
QByteArray HttpServer::receiveBody(HANDLE requestQueue, HTTP_REQUEST_ID requestId)
{
    QByteArray body;
...
void HttpServer::sendResponse(quint64 requestId, int statusCode, const QByteArray &body)
{
    ...
    HttpSendHttpResponse(
        m_requestQueue, requestId, 0, &response,
        nullptr, &bytesSent, nullptr, 0, nullptr, nullptr
    );
}
...
QString HttpServer::parseMethod(HTTP_VERB verb)
{
    switch (verb) {
...
QString HttpServer::parseUrl(const HTTP_COOKED_URL &cookedUrl)
{
